...
public class Elucidation {
    ...
    public static List<DataSet> findFragments(final Spectrum querySpectrum, final List<DataSet> compoundDataSets, final double shiftTol){
        final List<DataSet> fragments = new ArrayList<>();

        Assignment matchAssignment;
        for (final DataSet dataSet : compoundDataSets){
            matchAssignment = Matcher.matchSpectra(dataSet.getSpectrum(), querySpectrum, 0, 0, shiftTol);
